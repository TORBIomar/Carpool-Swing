...
public class DriverDashboard extends JFrame {
    ...
    private JButton createTripButton, updateTripButton, deleteTripButton, logoutButton, uploadDocumentsButton,
            viewHistoryButton, consultTripsButton, viewPassengersButton, submitReclamationButton;
    ...
    private void initializeComponents() {
        ...
        viewHistoryButton = new JButton("View Trip History");
        consultTripsButton = new JButton("Consult Trips");
        viewPassengersButton = new JButton("View Passengers");
        submitReclamationButton = new JButton("Submit Reclamation");

        JButton[] buttons = {createTripButton, updateTripButton, deleteTripButton, logoutButton,
                uploadDocumentsButton, viewHistoryButton, consultTripsButton, viewPassengersButton, submitReclamationButton};
        for (JButton button : buttons) {
            button.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
            button.setPreferredSize(new Dimension(100, 25)); // Smaller buttons
            button.setFocusPainted(false);
            button.setMargin(new Insets(2, 5, 2, 5)); // Reduced margins
        }
    ...
    private void setupLayout() {
        ...
        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 5, 5)); // Reduced gaps
        ...
        buttonPanel.add(consultTripsButton);
        ...
        if ("BOTH".equals(currentUser.getRole())) {
            ...
            switchButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
            switchButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
            switchButton.setFocusPainted(false);
            switchButton.setMargin(new Insets(2, 5, 2, 5)); // Reduced margins
    ...
    private void setupEventHandlers() {
        ...
        viewHistoryButton.addActionListener(e -> viewTripHistory());
        consultTripsButton.addActionListener(e -> consultTrips());
        viewPassengersButton.addActionListener(e -> viewAllPassengers());
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                ...
                while (rs.next()) {
                    Object[] row = {
                            rs.getInt("id"),
                            rs.getString("departure") != null ? rs.getString("departure") : "",
                            rs.getString("destination") != null ? rs.getString("destination") : "",
                            rs.getDate("date") != null ? sdfDate.format(rs.getDate("date")) : "",
                            rs.getTime("time") != null ? sdfTime.format(rs.getTime("time")) : "",
    ...
    private void openCreateTripDialog() {
        ...
        saveButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        saveButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
        JButton cancelButton = new JButton("Cancel");
        cancelButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
    ...
    private void openEditTripDialog(int tripId) {
        ...
        SimpleDateFormat sdfDate = new SimpleDateFormat("yyyy-MM-dd");
        JTextField dateField = new JTextField(tripData[3] != null ? sdfDate.format((java.sql.Date) tripData[3]) : "", 20);
        SimpleDateFormat sdfTime = new SimpleDateFormat("HH:mm");
        JTextField timeField = new JTextField(tripData[4] != null ? sdfTime.format((java.sql.Time) tripData[4]) : "", 20);
    ...
    private void showTripDetailsDialog(int tripId) {
        ...
        gbc.gridy++; dialog.add(new JLabel("Date: " + (tripData[3] != null ? sdfDate.format((java.sql.Date) tripData[3]) : "N/A")), gbc);
        gbc.gridy++; dialog.add(new JLabel("Time: " + (tripData[4] != null ? sdfTime.format((java.sql.Time) tripData[4]) : "N/A")), gbc);
        ...
        editButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        editButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
        editButton.addActionListener(e -> {
            dialog.dispose();
            openEditTripDialog(tripId);
        });
        JButton deleteButton = new JButton("Delete");
        deleteButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        deleteButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
        deleteButton.addActionListener(e -> {
            dialog.dispose();
            deleteTrip(tripId);
        });
        JButton closeButton = new JButton("Close");
        closeButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        closeButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    model.addRow(new Object[]{
                            rs.getInt("id"),
                            rs.getString("passenger_name") != null ? rs.getString("passenger_name") : "",
    ...
    private String getApplicationStatus() {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            pstmt.setInt(1, currentUser.getId());
            try (ResultSet rs = pstmt.executeQuery()) {
                return rs.next() ? rs.getString("application_status") != null ? rs.getString("application_status") : "NOT_APPLIED" : "NOT_APPLIED";
            }
        } catch (SQLException ex) {
    ...
    private void openUploadDocumentsDialog() {
        ...
        uploadIdCardButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        uploadIdCardButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
        ...
        uploadVehicleRegButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        uploadVehicleRegButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
    ...
    private void openSubmitReclamationDialog() {
        ...
        submitButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        submitButton.setPreferredSize(new Dimension(100, 25)); // Smaller button
        JButton cancelButton = new JButton("Cancel");
        cancelButton.setFont(new Font("Arial", Font.PLAIN, 12)); // Reduced font size
        cancelButton.setPreferredSize(new Dimension(100, 25)); // Smaller button

        gbc.gridx = 0; gbc.gridy = 0; dialog.add(new JLabel("Reclamation Description:"), gbc);
        gbc.gridx = 1; dialog.add(descriptionScroll, gbc);
    ...
    private void viewTripHistory() {
        JDialog dialog = new JDialog(this, "Trip History", true);
        dialog.setSize(600, 400);
        dialog.setLocationRelativeTo(this);
        dialog.setLayout(new BorderLayout());

        String[] cols = {"Trip ID", "Departure", "Destination", "Date", "Time", "Status"};
        DefaultTableModel model = new DefaultTableModel(cols, 0);
        JTable table = new JTable(model);
        table.setRowHeight(25);
        JScrollPane scroll = new JScrollPane(table);
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement pstmt = conn.prepareStatement(
                     "SELECT id, departure, destination, date, time, status FROM trips WHERE driver_id = ? ORDER BY date DESC")) {
            pstmt.setInt(1, currentUser.getId());
            try (ResultSet rs = pstmt.executeQuery()) {
                SimpleDateFormat sdfDate = new SimpleDateFormat("yyyy-MM-dd");
                SimpleDateFormat sdfTime = new SimpleDateFormat("HH:mm");
            ...
            JOptionPane.showMessageDialog(this, "Error loading trip history: " + ex.getMessage(),
                    "Database Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void consultTrips() {
        JDialog dialog = new JDialog(this, "Consult Trips", true);
        dialog.setSize(600, 400);
        dialog.setLocationRelativeTo(this);
        dialog.setLayout(new BorderLayout());

        String[] cols = {"Trip ID", "Departure", "Destination", "Date", "Time", "Seats Available"};
        DefaultTableModel model = new DefaultTableModel(cols, 0);
        JTable table = new JTable(model);
        table.setRowHeight(25);
        JScrollPane scroll = new JScrollPane(table);
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement pstmt = conn.prepareStatement(
                     "SELECT id, departure, destination, date, time, seats_available FROM trips WHERE driver_id = ? ORDER BY date ASC")) {
            pstmt.setInt(1, currentUser.getId());
            try (ResultSet rs = pstmt.executeQuery()) {
                SimpleDateFormat sdfDate = new SimpleDateFormat("yyyy-MM-dd");
                SimpleDateFormat sdfTime = new SimpleDateFormat("HH:mm");
            ...
            JOptionPane.showMessageDialog(this, "Error consulting trips: " + ex.getMessage(),
                    "Database Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void viewAllPassengers() {
        JDialog dialog = new JDialog(this, "All Passengers", true);
        dialog.setSize(600, 400);
        dialog.setLocationRelativeTo(this);
        dialog.setLayout(new BorderLayout());

        String[] cols = {"Passenger ID", "Name", "Email", "Phone"};
        DefaultTableModel model = new DefaultTableModel(cols, 0);
        JTable table = new JTable(model);
        table.setRowHeight(25);
        JScrollPane scroll = new JScrollPane(table);
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement pstmt = conn.prepareStatement(
                     "SELECT id, name, email, phone FROM users WHERE user_type = 'PASSENGER' OR user_type = 'BOTH'")) {
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    model.addRow(new Object[]{
                            rs.getInt("id"),
